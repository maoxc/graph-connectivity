...
fromEdges edges = Graph children parents allEdges vertices
  ...
  where
    parsedEdges =
      List.foldl
        (\acc (u, v) ->
           if u == v || elem (u, v) acc || elem (v, u) acc
             then acc
             else (u, v) : acc)
        []
        edges
    ...
    (children, parents) =
      List.foldl
        (\(map1, map2) (u, v) -> (addNeighbour u v map1, addNeighbour v u map2))
        (Map.empty, Map.empty)
        allEdges
...
edgeConnectivity inputEdges =
  minimum
    [ initGoldberg (Graph children Map.empty edges vertices) s t
    | (s, t) <- pairs vertices
    ]
  where
...
vertexConnectivity inputEdges =
  minimum
    [ initGoldberg (Graph newAdjMap newInvMap newEdges newVertices) s (-t)
    | (s, t) <- pairs vertices
    ]
  where
    (Graph children parents edges vertices) = fromEdges inputEdges
    newAdjMap =
      List.foldl
        (\acc v ->
           Map.insert (-v) [v] (Map.insert v (List.map (* (-1)) (acc ! v)) acc))
        children
        vertices
    newVertices = vertices ++ List.map (* (-1)) vertices
    newInvMap =
      List.foldl
        (\acc v -> Map.insert (-v) (List.map (* (-1)) (newAdjMap ! v)) acc)
        Map.empty
        newVertices
    newEdges =
      List.map (\x -> (-x, x)) vertices ++ List.map (\(u, v) -> (u, -v)) edges
...
goldberg children parents flow heights (Queue [] []) s t =
  excess t children flow
goldberg children parents flow heights q s t =
  if not (List.null pushableEdges)
 -- Perform push
    then let v = head pushableEdges
             updatedFlow
        -- Push flow along forward edge
               | (if member (v, u) flow
                    then flow ! (v, u)
                    else 0) ==
                   1 = Map.insert (v, u) 0 flow
        -- Reverse flow along backward edge
               | otherwise = Map.insert (u, v) 1 flow
             updatedQueue
               | v /= s &&
                   v /= t &&
                   excess v children updatedFlow > 0 &&
                   not (queued v poppedQueue) &&
                   excess u children updatedFlow > 0 &&
                   not (queued u poppedQueue) = push v (push u poppedQueue)
               | v /= s &&
                   v /= t &&
                   excess v children updatedFlow > 0 &&
                   not (queued v poppedQueue) = push v poppedQueue
               | excess u children updatedFlow > 0 && not (queued u poppedQueue) =
                 push u poppedQueue
 ...
 -- Relabel
           ...
           parents
           flow
           (Map.insert u (1 + (heights ! u)) heights)
           q
           s
           t
  where
 ...
 -- A bit is used to determine the direction of the edge {u,v} so we know what kind of push to perform along the edge
    pushableEdges =
      List.filter
        (\v -> (residual (u, v) flow > 0) && (heights ! u > heights ! v)) $
      (children ! u) ++
      (if member u parents
         then parents ! u
         else [])
...
addNeighbour u v children
  | Map.member u children = Map.insert u (v : (children ! u)) children
...
excess u children flow =
  sum
    [ if u `elem` (children ! v)
      then flow ! (v, u)
      else 0
    | v <- Map.keys children
    ] -
  sum [flow ! (u, v) | v <- children ! u]
